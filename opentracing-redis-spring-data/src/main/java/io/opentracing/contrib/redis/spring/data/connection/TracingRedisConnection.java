...
public class TracingRedisConnection implements RedisConnection {
  ...
  <T> T doInScope(String command, Supplier<T> supplier) {
    return TracingHelper.doInScope(command, supplier, withActiveSpanOnly, tracer);
  }
  ...
  public void set(byte[] key, byte[] value) {
    doInScope(RedisCommand.SET, () -> connection.set(key, value));
  }

  @Override
  public void set(byte[] key, byte[] value, Expiration expiration, SetOption option) {
    doInScope(RedisCommand.SET, () -> connection.set(key, value, expiration, option));
  }
  ...
  public void setEx(byte[] key, long seconds, byte[] value) {
    doInScope(RedisCommand.SETEX, () -> connection.setEx(key, seconds, value));
  }

  @Override
  public void pSetEx(byte[] key, long milliseconds, byte[] value) {
    doInScope(RedisCommand.PSETEX, () -> connection.pSetEx(key, milliseconds, value));
  }

  @Override
  public void mSet(Map<byte[], byte[]> tuple) {
    doInScope(RedisCommand.MSET, () -> connection.mSet(tuple));
  }
